...
Clearly the average estimate of mu\_hat is approximately equal to true
mu in this 5000 times simulation for mu = 0,1,2,3,4,5,6
...
Across test for which the null hypothesis is rejected, when true mu =
0,1,2,3, which means true mu is close to the null hypothesis(mu = 0),
the average estimate of mu\_hat is not equal to true value of mu. This
is because mu\_hats for those null hypothesis is rejected are biased
from population mean since the proportion of rejecting mu = 0 is not
high enough. In a special case when population mean = 1, mu\_hat needs
to be larger to reject H0 (which is mu = 0), so average estimate mu\_hat
is above 2.

When true mu is so large that the null hypothesis is always rejected,
the average estimate of mu\_hat is approximately equal to true value of
mu. This is because the average estimate of mu\_hat basically represent
the estimate of mu in 5000 samples simulated from normal distribution,
which equals to the population mean.
